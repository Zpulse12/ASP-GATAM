...
namespace UnitTesting.CQRSTest.Question
{
    ...
    public class CreateQuestionCommandTest
    {
        ...
        {
            Gatam.Domain.Question question = new Gatam.Domain.Question() { QuestionTitle = "Wat heb je gegeten vandaag?", QuestionAnswer = "Pizza" };
            ...
            Assert.AreEqual("Pizza", result.QuestionAnswer);
        }
        [TestMethod]
        public async Task QuestionTitle_shouldBe_Uniform()
        {
            Gatam.Domain.Question question = new Gatam.Domain.Question() { QuestionTitle = "hoeveel eitjes wil je bij je spek", QuestionAnswer = "2" };
            CreateQuestionCommand questionCommand = new CreateQuestionCommand() { question = question };
            _unitOfWork.Setup(uow => uow.QuestionRepository.Create(It.IsAny<Gatam.Domain.Question>())).Returns(Task.FromResult(question));
            Gatam.Domain.Question result = await _handler.Handle(questionCommand, CancellationToken.None);
            ...
            Assert.AreEqual("2", result.QuestionAnswer);
        }

        [TestMethod]
        public async Task Handler_shouldFail_onEmptyTitle()
        {
            Gatam.Domain.Question question = new Gatam.Domain.Question() { QuestionTitle = "", QuestionAnswer = "2" };
            CreateQuestionCommand questionCommand = new CreateQuestionCommand() { question = question };
        ...
        {
            Gatam.Domain.Question question = new Gatam.Domain.Question() { QuestionTitle = "Welk fruit vind je het lekkerst?", QuestionAnswer = "" };
            CreateQuestionCommand questionCommand = new CreateQuestionCommand() { question = question };
        ...
        {
            Gatam.Domain.Question question = new Gatam.Domain.Question() { QuestionTitle = "Brood?", QuestionAnswer = "2" };
            CreateQuestionCommand questionCommand = new CreateQuestionCommand() { question = question };
